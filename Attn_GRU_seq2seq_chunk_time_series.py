...
class helper_functions():
    ...
    @staticmethod
    def seq2seq_training(model, train_seg, optimiser, lossfunction, clip,teacher_forcing_ratio):
      model.train()
      epoch_loss = 0
      for train_sample in train_seg:
        X_local = train_sample[0]
        y_local = train_sample[1]

        # print('load')
        # print(X_local.size())
        # print(y_local.size())

        local_batch, local_labels = X_local.unsqueeze(1).to(device), y_local.unsqueeze(1).to(device) 
        ...
        local_output = model(seq2seq_input=local_batch, target=local_labels, teacher_forcing_ratio = teacher_forcing_ratio)
    ...
    @staticmethod
    def seq2seq_evaluate(model, test_seg, lossfunction):
      model.eval()
      epoch_loss = 0
      with torch.no_grad():
        for test_sample in test_seg:
          X_local = test_sample[0]
          y_local = test_sample[1]
          local_batch, local_labels = X_local.unsqueeze(1).to(device), y_local.unsqueeze(1).to(device) 
          local_labels = local_labels.unsqueeze(2)
    ...
    def seq2seq_running(grid,model,optimiser,lossfunction,X_train,y_train,X_test,y_test,teacher_forcing_ratio,train_seg_len,test_seg_len):
      ...
      # data loader
      training_Loader = _formatted_Dataset(X_train,y_train)
      train_seg = training_Loader.timeSeires_chunker(train_seg_len,test_seg_len)
      test_Loader = _formatted_Dataset(X_test,y_test)
      test_seg = test_Loader.timeSeires_chunker(train_seg_len,test_seg_len)

      for epoch in range(grid['max_epochs']):

        train_loss = helper_functions.seq2seq_training(model, train_seg, optimiser, lossfunction, grid['clip'],teacher_forcing_ratio)
        valid_loss = helper_functions.seq2seq_evaluate(model, test_seg, lossfunction)
...
class _formatted_Dataset(): 
  ...
  def timeSeires_chunker(self, \
    ...
    for i in range(0,N_samples,seq_len):
      ...
      y_segment = self._y[i:min(i+seq_len,N_samples)]
      
      if len(y_segment) == seq_len: # complete data
        segmentation.append([X_segment,y_segment])

      else: # incomplete data
        residual = [X_segment,y_segment]

    if shuffle:
      random.shuffle(segmentation)

    return segmentation #+ residual     throw away residual
