...
def show_parameter():
    ...
    param = {'max_epochs':64,
            ...
            'ENC_DROPOUT':0,
            'DEC_DROPOUT':0,
            'chunk_seq_len':10,        # chunk time series sequence length
            'batch_size':1,
            'device':device}
...
class helper_functions():
    ...
    def instan_things(**kwargs):
        grid = {'max_epochs': kwargs['max_epochs'],
                ...
                'chunk_seq_len': kwargs['chunk_seq_len'],
    ...
    def seq2seq_running(grid, model, optimiser, lossfunction, X_train, y_train, X_test, y_test,
                        teacher_forcing_ratio, chunk_seq_len):
...
class _Dataset():
    ...
    def bag_of_timeSeries_chunk(self,
                                ...
                                shuffle=True):
        '''
        future mode: given [t,t+chunk_seq_len] indicator data + price data, 
                                predict [t+chunk_seq_len,t+2*chunk_seq_len]price

        output class attributes:
            self.X  [N_sample, chunk_seq_len, N_feature]
            self.y  [N_sample, chunk_seq_len]
        ...
        for i in range(0, N_samples, chunk_seq_len):
            X_segment = self._X[i:min(
                i+2*chunk_seq_len, N_samples)]
            y_segment = self._y[i:min(
                i+2*chunk_seq_len, N_samples)].unsqueeze(1)
        ...
        if shuffle:
            self.shuffler()
        N_samples = self._segment.size(0)
